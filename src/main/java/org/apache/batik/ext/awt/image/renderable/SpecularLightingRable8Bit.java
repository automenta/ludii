...
public class SpecularLightingRable8Bit extends AbstractColorInterpolationRable implements SpecularLightingRable
{
    ...
    public SpecularLightingRable8Bit(final Filter src, final Rectangle2D litRegion, final Light light, final double ks, final double specularExponent, final double surfaceScale, final double[] kernelUnitLength) {
        super(src, null);
    ...
    public RenderedImage createRendering(RenderContext rc) {
        ...
        if (this.kernelUnitLength != null) {
            if (scaleX >= 1.0f / this.kernelUnitLength[0]) {
                scaleX = 1.0f / this.kernelUnitLength[0];
            }
            if (scaleY >= 1.0f / this.kernelUnitLength[1]) {
