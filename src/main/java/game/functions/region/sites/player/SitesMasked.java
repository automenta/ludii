...
@Hide
public final class SitesMasked extends BaseRegionFunction
{
    private static final long serialVersionUID = 1L;
    private Region precomputedRegion;
    private final IntFunction index;
    
    public SitesMasked(@Or @Opt final Player player, @Or @Opt final RoleType role, @Opt final SiteType type) {
    ...
    public Region eval(final Context context) {
        ...
        if (pid < 1 || pid > context.game().players().size()) {
            System.out.println("SitesMasked: Bad player index.");
            return new Region();
        }
        ...
        for (int id = 0; id < context.containers().length; ++id) {
            ...
            for (int site = from; site < from + size; ++site) {
                if (cs.isMasked(site, pid, this.type)) {
                    sites.add(site);
                }
            }
        }
