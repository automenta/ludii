...
public class Tiling4612 extends Basis
{
    ...
    public Tiling4612(final DimFunction dim) {
        ...
        for (int s = 0; s < 12; ++s) {
            ...
            Tiling4612.ref[s][0] = r * Math.cos(theta);
            Tiling4612.ref[s][1] = r * Math.sin(theta);
        }
    ...
    public static Point2D.Double xy(final int row, final int col) {
        final double dx = 4.7320508;
        final double dy = 4.7320508 * Math.sqrt(3.0) / 2.0;
        return new Point2D.Double((col - 0.5 * row) * 4.7320508, row * dy);
    }
