...
@Hide
public final class CountDegreeProduct extends BaseIntFunction
{
    private static final long serialVersionUID = 1L;
    private final IntFunction who;
    
    public CountDegreeProduct(@Or final IntFunction who, @Or final RoleType role) {
        int numNonNull = 0;
        if (who != null) {
            ++numNonNull;
        }
    ...
    public int eval(final Context context) {
        ...
        for (int j = 0; j < totalVertices; ++j) {
            if (degreeInfo[j].cardinality() != 0) {
                product *= degreeInfo[j].cardinality();
            }
        }
        return product;
    }
