...
public final class CountLiberties extends BaseIntFunction
{
    ...
    public CountLiberties(@Opt final SiteType type, @Opt @Name final IntFunction at, @Opt final Direction directions, @Opt @Name final BooleanFunction If) {
    ...
    public int eval(final Context context) {
        ...
        for (int indexGroup = 0; indexGroup < groupSites.size(); ++indexGroup) {
            ...
            final TopologyElement element = topology.getGraphElements(this.type).get(siteGroup);
            ...
            for (final AbsoluteDirection direction : directionsElement) {
                ...
                for (final Step step : steps) {
                    final int to = step.to().id();
                    if (!groupSites.contains(to) && !liberties.contains(to) && cs.what(to, this.type) == 0) {
                        liberties.add(to);
                    }
                }
            }
