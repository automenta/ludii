...
@Hide
public final class SetMasked extends Effect
{
    ...
    public SetMasked(@Opt final SiteType type, @Or final IntFunction site, @Or final RegionFunction region, @Opt @Or2 final Player who, @Opt @Or2 final RoleType role, @Opt @Name final BooleanFunction stack, @Opt final Then then) {
        super(then);
        if (region != null) {
        ...
        if (stack) {
            for (final int site : sites) {
                ...
                for (int sizeStack = cs.sizeStack(site, SiteType.Cell), lvl = 0; lvl < sizeStack; ++lvl) {
                    if (who != -1) {
                        actions.add(new ActionSetMasked(this.type, site, lvl, who));
                    }
                    else {
                        for (int pid = 1; pid < context.game().players().size(); ++pid) {
                            actions.add(new ActionSetMasked(this.type, site, lvl, pid));
                        }
                    }
                }
            }
            ...
            for (final int site : sites) {
                actions.add(new ActionSetMasked(this.type, site, -1, who));
            }
        }
