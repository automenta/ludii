...
public final class Constants {
    ...
    public static List<BitSet>[][] combos = null;
    ...
    public static void createCombos() {
        combos = new ArrayList[21][21];
        for (int m = 0; m < 21; ++m) {
            for (int n = 0; n < 21; ++n) {
                Constants.combos[m][n] = new ArrayList<>();
            }
        }
        for (int seed = 0; seed < 0x100000; ++seed) {
            int on;
            for (int n = on = Integer.bitCount(seed); n < 21; ++n) {
                if (seed >= 1 << n) continue;
    ...
    public static void dumpCombos() {
        for (int m = 0; m < 21; ++m) {
            for (int n = 0; n < 21; ++n) {
                System.out.println("\nm=" + m + ", n=" + n + ":");
                if (combos[m][n].isEmpty()) continue;
                for (BitSet bits : combos[m][n]) {
                    int index = 0;
                    for (int b = 0; b < n; ++b) {
