...
{
    public ButtonEnd(final int cx, final int cy, final int sx, final int sy) {
    ...
    protected boolean isEnabled() {
        ...
        return Manager.savedTrial() != null && Manager.savedTrial().moves().size() > context.trial().moves().size() && SettingsNetwork.getActiveGameId() == 0;
    }
    ...
    public void press() {
        if (this.isEnabled()) {
            ...
            ToolView.jumpToMove(TrialUtil.getInstanceEndIndex(context));
        }
    }
}
